...
After switching tiers, restart the proxy: `ocx litellm down && oc litellm up`.
...
`mode: primary` is used only for the four orchestrators above. Claude Code does not support `mode` or `permission` — `ocx agents sync` strips them automatically.
